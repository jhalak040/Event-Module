		...
		try {
			...
			if (message.getStatusSelect() == MessageRepository.STATUS_SENT) {
				...
				if (message.getSentByEmail()) {
					...
					String emails = message.getTemplate().getToRecipients();
					List<String> emailAddressList = Lists.newArrayList(Arrays.asList(emails.split(",")));
					for (String string : emailAddressList) {
						List<EventRegistration> emailList = eventRegRepo.all().filter("self.email =(?)", string)
								.fetch();
						for (EventRegistration eventRegistration : emailList) {
