...
public class TemplateMessageServiceImpl implements TemplateMessageService {
	...
	public Message generateMessage(Long objectId, String model, String tag, Template template)
			throws ClassNotFoundException, InstantiationException, IllegalAccessException, AxelorException,
			IOException {
		...
		if (metaModel != null) {
			if (!model.equals(metaModel.getFullName())) {
				throw new AxelorException(TraceBackRepository.CATEGORY_INCONSISTENCY,
						I18n.get(IExceptionMessage.TEMPLATE_SERVICE_3), template.getMetaModel().getFullName());
			}
		...
		String content = "", subject = "", from = "", replyToRecipients = "", toRecipients = "", ccRecipients = "",
				bccRecipients = "", addressBlock = "";
		int mediaTypeSelect;

		Event event = eventRepo.all().filter("self.id=?", objectId).fetchOne();
		List<EventRegistration> emalList = event.getEventRegistration();
		List<String> emailAddressList = Lists.newArrayList();
		for (EventRegistration eventRegistration : emalList) {
			if (eventRegistration.getEmail() != null) {
				emailAddressList.add(eventRegistration.getEmail());
			}
		}
		String email = emailAddressList.toString();
		String emailLists = email.substring(1, email.length() - 1);
		template.setToRecipients(emailLists);
		System.out.println(template.getToRecipients());
		...
		Message message = messageService.createMessage(model, Long.valueOf(objectId).intValue(), subject, content,
				getEmailAddress(from), getEmailAddresses(replyToRecipients), getEmailAddresses(toRecipients),
				getEmailAddresses(ccRecipients), getEmailAddresses(bccRecipients), null, addressBlock, mediaTypeSelect,
				getMailAccount());
	...
	@Override
	public Message generateAndSendMessage(Model model, Template template) throws MessagingException, IOException,
			AxelorException, ClassNotFoundException, InstantiationException, IllegalAccessException {
	...
	public Set<MetaFile> getMetaFiles(Template template) throws AxelorException, IOException {
		...
		List<DMSFile> metaAttachments = Query.of(DMSFile.class).filter("self.relatedId = ?1 AND self.relatedModel = ?2",
				template.getId(), EntityHelper.getEntityClass(template).getName()).fetch();
	...
	protected List<EmailAddress> getEmailAddresses(String recipients) {
		/*
		 * List<EmailAccount> emailList = (List<EmailAccount>) new EmailAccount();
		 * List<EventRegistration> eventList =
		 * eventRegRepo.all().filter("self.email IN= (?)").fetch();
		...
		/*
		 * List<EventRegistration> emalList = eventRegRepo.all().filter("self").fetch();
		 */
		// Set<EmailAddress> emailList = (Set<EmailAddress>)
		// emalList.stream().map(EventRegistration::getEmail);
