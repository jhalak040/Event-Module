...
public class MailAccountServiceImpl implements MailAccountService {
	...
	public void checkDefaultMailAccount(EmailAccount mailAccount) throws AxelorException {
		...
		if (mailAccount.getIsDefault()) {
			...
			if (serverTypeSelect == EmailAccountRepository.SERVER_TYPE_SMTP) {
				...
				query += " AND (self.serverTypeSelect = " + EmailAccountRepository.SERVER_TYPE_IMAP + " OR "
						+ "self.serverTypeSelect = " + EmailAccountRepository.SERVER_TYPE_POP + ") ";
			}
	...
	public EmailAccount getDefaultSender() {
		...
		return mailAccountRepo.all()
				.filter("self.isDefault = true AND self.serverTypeSelect = ?1", EmailAccountRepository.SERVER_TYPE_SMTP)
				.fetchOne();
	}
	...
	public EmailAccount getDefaultReader() {
		...
		return mailAccountRepo.all()
				.filter("self.isDefault = true " + "AND (self.serverTypeSelect = ?1 OR self.serverTypeSelect = ?2)",
						EmailAccountRepository.SERVER_TYPE_IMAP, EmailAccountRepository.SERVER_TYPE_POP)
				.fetchOne();
	}
	...
	public void checkMailAccountConfiguration(EmailAccount mailAccount) throws AxelorException, Exception {
		...
		try {
			...
			if (mailAccount.getServerTypeSelect().equals(EmailAccountRepository.SERVER_TYPE_SMTP)) {
				...
				transport.connect(mailAccount.getHost(), mailAccount.getPort(), mailAccount.getLogin(),
			...
			throw new AxelorException(e, mailAccount, TraceBackRepository.CATEGORY_CONFIGURATION_ERROR,
	...
	public com.axelor.mail.MailAccount getMailAccount(EmailAccount mailAccount) {
		...
		if (serverType == EmailAccountRepository.SERVER_TYPE_SMTP) {
			...
			account = new ImapAccount(mailAccount.getHost(), mailAccount.getPort().toString(), mailAccount.getLogin(),
					getDecryptPassword(mailAccount.getPassword()), getSecurity(mailAccount));
		} else {
			account = new Pop3Account(mailAccount.getHost(), mailAccount.getPort().toString(), mailAccount.getLogin(),
					getDecryptPassword(mailAccount.getPassword()), getSecurity(mailAccount));
		}
	...
	public int fetchEmails(EmailAccount mailAccount, boolean unseenOnly) throws MessagingException, IOException {
		...
		log.debug("Fetching emails from host: {}, port: {}, login: {} ", mailAccount.getHost(), mailAccount.getPort(),
		...
		if (mailAccount.getServerTypeSelect().equals(EmailAccountRepository.SERVER_TYPE_IMAP)) {
			account = new ImapAccount(mailAccount.getHost(), mailAccount.getPort().toString(), mailAccount.getLogin(),
					mailAccount.getPassword(), getSecurity(mailAccount));
		} else {
			account = new Pop3Account(mailAccount.getHost(), mailAccount.getPort().toString(), mailAccount.getLogin(),
					mailAccount.getPassword(), getSecurity(mailAccount));
		}
